...
function _addAttachmentWrap(attachmentName, add) {
    let to = typeof(attachmentName);
    if (to === "number") {
        _addAttachment(this, attachmentName, !add);
    } else if (to === "string") {
        _addAttachment(this, mp.joaat(attachmentName), !add);
    }
}
...
mp.events.add("ped:create", (ped) => {
    ped._attachments = [];
    ped.addAttachment = _addAttachmentWrap;
    ped.hasAttachment = _hasAttachment;
});
